...
LDFLAGS = 
LDFLAGS_PRIV = -Llib -l${CKVO} -L$(MEMORY_MANAGEMENT_LIB)/lib -lmemorymanagement -lpthread $(LDFLAGS)

WLFLAGS=-Wl,-rpath,lib$(CKVO).so.$(WDMAJORVERSION)
BIN = bin
INC = include
OBJ = obj
SRC = src
...
WDSTATIC = ${LIB}/lib${CKVO}.a
libckvo : directories libmemorymanagement $(WDSTATIC)

WDMAJORVERSION=0
WDMINORVERSION=1
WDRELEASENUMBER=1
WDSONAME = ${LIB}/lib${CKVO}.so.$(WDMAJORVERSION)
WDREALNAME = ${LIB}/lib${CKVO}.so.$(WDMAJORVERSION).$(WDMINORVERSION).$(WDRELEASENUMBER)
WDSHARED = ${LIB}/lib${CKVO}.so
FIRSTLINK = $(patsubst $(LIB)/%,%,$(WDREALNAME))
SECONDLINK = $(patsubst $(LIB)/%,%,$(WDSONAME))
libckvoshared : directories $(WDSHARED)
	@ln -s $(FIRSTLINK) $(WDSONAME) 
	@ln -s $(SECONDLINK) $(WDSHARED) 

TESTS = $(patsubst $(TEST)/%.c,$(BIN)/%,$(wildcard $(TEST)/*.c))
tests : directories lib$(CKVO) $(TESTS)
...
${LIB}/lib${CKVO}.a : $(patsubst ${SRC}/%.c,${OBJ}/%.o,$(wildcard ${SRC}/*.c))
	${AR} r ${LIB}/lib${CKVO}.a $?

${LIB}/lib${CKVO}.so : $(WDSONAME)

${LIB}/lib${CKVO}.so.$(WDMAJORVERSION) : $(WDREALNAME)

${LIB}/lib${CKVO}.so.$(WDMAJORVERSION).$(WDMINORVERSION).$(WDRELEASENUMBER) : $(patsubst ${SRC}/%.c,${OBJ}/%.o,$(wildcard ${SRC}/*.c))
